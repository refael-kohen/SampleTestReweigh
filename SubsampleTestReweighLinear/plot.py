...
from SubsampleTestReweighLinear.utils import Const  # Not !!! import np from utils (this script cannot work with cupy)

plt.rcParams.update({'font.size': 10})


class AggregateOutputFiles:
    def __init__(self, output_path, log_path, repetitions_num, run_parallel):
    ...
    def aggregate_parallel_logs(self, sample_size_S):
        ...
        log_names = glob(os.path.join(self.log_path, '00*' + '_' + str(sample_size_S) + '.log'))
        log_w_names = glob(os.path.join(self.log_path, '00*' + '_' + str(sample_size_S) + '_weights.log'))
    ...
    def aggregate_parallel_outputs_one_group(self, file_name_pre, sample_size_S):
        # in multiprocessing will be created different temp file for each repetition.
        file_names = glob(os.path.join(self.output_path, 'sampSize_' + str(sample_size_S), '00*' + file_name_pre+'*'))
        aggregated_file_name = os.path.join(self.output_path, 'sampSize_' + str(sample_size_S),
    ...
    def aggregate_parallel_files(self, sample_size_S):
        self.aggregate_parallel_logs(sample_size_S)
...
class Plot:
    def __init__(self, output_path, log_path, plot_path, sample_sizes, skip_iterations=50, run_parallel=False):
        self.output_path = output_path
        self.log_path = log_path
    ...
    #     # after insertion of 'sample size' column in first column, and considering the last column - 'succeed'
    ...
    def get_file_list(self, file_name_pre):
        ...
        for sample_size_S in self.sample_sizes:
            file_name = glob(
                os.path.join(self.output_path, 'sampSize_' + str(sample_size_S), '[!00]*' + file_name_pre + '*'))
    ...
    def concatenate_files_from_sample_sizes(self, file_name_pre):
        ...
        for filename, sample_size_S in zip(files_list, self.sample_sizes):
            ...
            df_one.insert(0, 'sample size', sample_size_S)
    ...
    def general_vs_iters(self, file_name_pre, col_name, label_graph=None):
        ...
        df_grouped_mean = df.groupby('sample size').mean().T
        df_grouped_std = df.groupby('sample size').std().T
        ...
        for sample_size in self.sample_sizes:
            ax = df_grouped_mean[sample_size].dropna().iloc[::-self.skip_iterations].iloc[::-1]. \
                dropna().plot(label=sample_size, linewidth=0.7)
    ...
    def general_vs_sample_size(self, file_name_pre, file_out_name, label_graph=None):
        ...
        df_sample_cols = df.pivot(columns='sample size', values=col_name)
        fig, ax = plt.subplots()
        mean = df_sample_cols.mean()
        std = df_sample_cols.std()
        ax.errorbar(x=df_sample_cols.columns, y=mean, yerr=std, marker='o')
