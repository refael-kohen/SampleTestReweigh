...
from SubsampleTestReweighLinear.LP_model import LP
from SubsampleTestReweighLinear.data_structures import DataStruct
from SubsampleTestReweighLinear.loggers import Logger
from SubsampleTestReweighLinear.mw_algorithm import MWTransferLearning
...
def run_repetition(v, model, log_str, ds_succeeded, ds_failed,
    ...
    # The true hypothesis is also random so we create it in each repetition
    true_hypo = Sample.get_true_hypothesis(alpha=v.PrecV.alpha, mean_k_t=v.SampleV.mean_k_t, std_k_t=v.SampleV.std_k_t,
                                           dim=v.SampleV.dim,
                                           noise=v.ModelV.real_hypo_noise, k_coord=v.SampleV.k_coord)
    logger.info('\t\tPrepare the target distribution')
    ...
    sample_s = Sample(mean=v.SampleV.mean_s, std=v.SampleV.std_s, mean_k=v.SampleV.mean_k_s, std_k=v.SampleV.std_k_s,
                    k_coord=v.SampleV.k_coord, dim=v.SampleV.dim).get_sample(v.SampCompV.sample_size_s)
    logger.info('\t\tSample from the source distribution {} examples'.format(v.SampCompV.sample_size_s))
    pdf_sample_s, pdf_sample_s_in_t = Sample.get_ppf(sample_s=sample_s, true_hypothesis=true_hypo,
                                                     mean_k_t=v.SampleV.mean_k_t, std_k_t=v.SampleV.std_k_t,
                                                     mean_k_s=v.SampleV.mean_k_s, std_k_s=v.SampleV.std_k_s, k=v.SampleV.k)
    labels_s = Sample.get_label(sample=sample_s, hypothesis=true_hypo[:-1], intercept=true_hypo[-1])
    mw_algo = MWTransferLearning(model=model, sample=sample_s, labels=labels_s, pdf_sample_s_in_t=pdf_sample_s_in_t,
                                 pdf_sample_s=pdf_sample_s, sq_oracle=sq_oracle,
                                 log_str='rep: {}, '.format(rep + 1) + log_str, logger=logger)
    succeeded, iter_num_idx, loss_s, loss_s_sub, loss_t, kl_dist, tv_dist, chisq_dist = mw_algo.run(
        print_freq=v.MWalgV.mw_print_freq, v=v)
...
def create_model(v):
    model = None
    if v.ModelV.model_name == Const.SVM:
        if v.PrivateV.private:
            # You can verify that the running algorithm is private by setting verbose=1
            model = PrivateSVM(loss='hinge', fit_intercept=True, penalty=v.ModelV.penalty,
                               alpha=1 / v.ModelV.regularization_c, verbose=0, private=v.PrivateV.private,
                               mean_noise=0,
                               sigam_noise=v.PrivateV.ss_noise_sigma, debug_noise=v.PrivateV.ss_noise_debug_mode,
                               num_threads_rand=v.PrivateV.ss_noise_num_threads_rand,
                               max_total_iterations_private=v.PrivateV.ss_noise_max_iter)
            # tol=1e-3, n_iter_no_change=5, max_iter=50, learning_rate='constant', eta0=30
        else:
            # model = LinearSVC(C=1000000000000, fit_intercept=True, dual=False)
            model = SGDClassifier(loss='hinge', fit_intercept=True, penalty=v.ModelV.penalty,
                                  alpha=1 / v.ModelV.regularization_c)
...
def run_experiments(v):
    ...
    logger.info(
        'The upper bound on sample S is: {}'.format(ceil(64 * log2(8 * (v.SampleV.chi_s + 1)) / (v.PrecV.alpha ** 2))))
