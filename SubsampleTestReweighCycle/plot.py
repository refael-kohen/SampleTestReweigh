...
from SubsampleTestReweighCycle.utils import Const  # Not !!! import np from utils (this script cannot work with cupy)
...
class AggregateOutputFiles:
    ...
    def aggregate_parallel_logs(self, sample_size_S):
        ...
        log_names = glob(os.path.join(self.log_path, '*' + '_' + str(sample_size_S) + '.log'))
        log_w_names = glob(os.path.join(self.log_path, '*' + '_' + str(sample_size_S) + '_weights.log'))
    ...
    def aggregate_parallel_outputs_one_group(self, file_name_pre, sample_size_S):
        ...
        # in multiprocessing will be created different temp file for each repetition.
        if self.private:
            file_names = glob(os.path.join(self.output_path, 'sgdIters_' + str(sample_size_S), '*' + file_name_pre + '*'))
            aggregated_file_name = os.path.join(self.output_path, 'sgdIters_' + str(sample_size_S),
                                                '_'.join(['agg', file_name_pre, str(sample_size_S)]) + '.txt')
        else:
            file_names = glob(os.path.join(self.output_path, 'sampSize_' + str(sample_size_S), '*' + file_name_pre + '*'))
...
class Plot:
    ...
    def get_file_list(self, file_name_pre):
        ...
        for sample_size_S in self.sample_sizes:
            if self.private:
                file_name = glob(
                    os.path.join(self.output_path, 'sgdIters_' + str(sample_size_S), 'agg*' + file_name_pre + '*'))
            else:
                file_name = glob(
                    os.path.join(self.output_path, 'sampSize_' + str(sample_size_S), 'agg*' + file_name_pre + '*'))
    ...
    def general_vs_iters(self, file_name_pre, col_name, label_graph=None):
        ...
        for sample_size in self.sample_sizes:
            ...
            std = data.std().mean()
            if self.privagte:
                ax.errorbar(x=data.index, y=data, yerr=data_std, errorevery=5, label=sample_size, linewidth=0.7)
            else:
