...
import numpy as np
from SubsampleTestReweighCycle.utils import *

private_params = False
...
class PrecVC:
    ...
    alpha = 0.01  # 0.01 # error on T


# Sample
class SampleVC:
    dim = 200   # must be even (in _sgd_fast.pyx)
    mean_s = 0.0
    std_s = 1.0
    mean_k_s = 0.0
    std_k_s = 1.0
    mean_t = 0.0
    std_t = 1.0
    k_coord = list(range(40, 46))
    frac_zero_label_t = 0.3  # fraction of examples in T that are 0-labeled (according that we consider the threshold)
...
class MWalgVC:
    ...
    if private_params:
        ...
        mw_max_iter = 30000  # 'ceil(64 * log2(8 * (v.SampleV.chi_s + 1)) / (v.PrecV.alpha ** 2))' # expected number of MW algorithm iterations depending on alpha
...
class ModelVC:
    ...
    early_stopping_n_iter = 1500  # check each such iterations of SGD if the es_score is achieved.
